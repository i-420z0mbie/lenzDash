...
const FeeManagement = () => {
  ...
  const [deleteConfirm, setDeleteConfirm] = useState({
    open: false,
    type: null, // 'structure' or 'item'
    id: null,
    name: '',
    structureId: null, // for item deletion, we need to know parent structure
  });
  ...
  const handleDeleteItem = async () => {
    ...
    try {
      ...
      setDeleteConfirm({ open: false, type: null, id: null, name: '', structureId: null });
      await fetchAll();
      // If the structure detail modal is open, refresh its data
  ...
  // Helper to render the delete button for items
  ...
  return (
    <div className="space-y-5 p-4 md:p-5 max-w-[1600px] mx-auto animate-fadeInUp">
        ...
        <div className="relative flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <p className="text-xxs text-slate-500 mt-0.5">
              Manage all fee structures, inspect full item lists, and edit or delete items and structures
            </p>
          </div>
          <button
            onClick={fetchAll}
                ...
                <div className="flex items-center justify-between mb-2">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <button
                      ...
                      className="text-xxs text-rose-600 font-medium hover:text-rose-700"
                    >
                      🗑 Delete structure
                    </button>
                  </div>
      ...
      {/* Delete Confirmation Modal (handles both structure and item) */}
      {deleteConfirm.open && (
        <Modal
          ...
          <div className="space-y-4">
            <p className="text-sm text-slate-700">
              {deleteConfirm.type === 'structure' ? (
                <>Are you sure you want to delete the fee structure for <strong>{deleteConfirm.name}</strong>?</>
              ) : (
                <>Are you sure you want to delete the fee item <strong>{deleteConfirm.name}</strong>?</>
              )}
            </p>
            <p className="text-xxs text-slate-500">
              {deleteConfirm.type === 'structure'
                ? 'This action cannot be undone. All fee items under this structure will also be deleted.'
            ...
            <div className="flex gap-2 pt-2">
              ...
              <button
                ...
                className="flex-1 py-1.5 text-xxs bg-rose-600 text-white rounded-lg hover:bg-rose-700 transition"
              >
                Yes, Delete
              </button>
            </div>
